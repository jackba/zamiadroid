...
public class ProjectXMLparser {
	...
	private ResearchControler rsCntrl;
	private boolean error=false;
	


	public ProjectXMLparser(ResearchControler rsC){
	...
	try {
         ...
         ProjectHandlerXML myExampleHandler = new ProjectHandlerXML(rsCntrl);
       		...
       		URL urlR = new URL(url); 
       		 ...
       		 xr.parse(new InputSource(urlR.openStream())); 
       	  	...
       	  	//  InputStream fis = c.getAssets().open("botanical_project.xml");
       	  	 ...
       	  	 rsCntrl.setMandatory();
       	  	 
       	  	 if(rsCntrl.getCitationType().equals("Botanical")){
       	  		 
	       	   	 InputStream fise = c.getAssets().open("botanical.xml");
	    	  	 xr.parse(new InputSource(fise));
 
       	  	 }
       	  	 else if(rsCntrl.getCitationType().equals("Ornithological")){
